...
class StructureCompanyProvider extends BaseProvider
{
	...
	const MODE_USERS_ONLY = 'usersOnly';
	const MODE_USERS_AND_DEPARTMENTS = 'usersAndDepartments';
	const ENTITY_ID = 'department'; 
	...
	public function __construct(array $options = [])
	{
		...
		$this->logger->record("__construct", [
			'raw_object' => $options
		]);
	...
	public function isAvailable(): bool
	{
		...
		$this->logger->record("Вызов isAvailable", [
			'raw_object' => UserProvider::isIntranetUser()
		]);
	...
	{
		$this->logger->record("Вызов getItems", [
			'raw_object' => $ids
		]);
	...
	{
		$this->logger->record("Вызов getSelectedItems", [
			'raw_object' => $this->getDepartments($ids, ['active' => null])
		]);
	...
	public function getDepartments(array $ids, array $options = []): array {
		...
		$this->logger->record("Вызов getDepartments", [
			'raw_object' => $items
		]);
	...
	public function fillDialog(Dialog $dialog): void
	{
		...
		if ($this->options['fillDepartmentsTab'] === true || $this->options['fillRecentTab'] === true)
		{
			...
			if (!$limitExceeded || $this->getSelectMode() === self::MODE_USERS_ONLY)
			{
				...
				$entity = $dialog->getEntity(entityId: 'department');
				if ($entity)
				{
		...
		$this->logger->record("Вызов fillDialog options", [
			'raw_object' => $this->options
		]);
		$this->logger->record("Вызов fillDialog", [
			'raw_object' => $departments
		]);
	}
	...
	public function getStructure(array $options = []): EO_Section_Collection
	{
		...
		$structure = SectionTable::getList([
		...
		// $this->logger->record("Вызов getStructure", [
		// 	'raw_object' => $structure,
		// ]);
	...
	{
		// $this->logger->record("Вызов getSelectMode", [
		// 	'raw_object' => $this->options
		// ]);
	...
	private function fillRecentDepartments(Dialog $dialog, EO_Section_Collection $departments)
	{
		...
		// $this->logger->record("Вызов fillRecentDepartments", [
		// 	'raw_object' => $departments
		// ]);
	...
	{
		$this->logger->record("Вызов getChildren", [
			'raw_object' => $parentItem
		]);
	...
	public function doSearch(SearchQuery $searchQuery, Dialog $dialog): void
	{
		...
		$this->logger->record("Вызов doSearch", [
			'raw_object' 		=> $searchQuery,
		]);

		$this->logger->record("Вызов doSearch", [
			'raw_object' 	=> $selectMode,
		]);
	...
	{
		$this->logger->record("Вызов getUserOptions", [
			'raw_object' => $dialog
		]);
	}
	private function fillDepartments(Dialog $dialog, EO_Section_Collection $departments, ?bool $forceDynamic = null)
	{
		...
		$this->logger->record("Вызов fillDepartments", [
			'raw_object' => $parents
		]);
