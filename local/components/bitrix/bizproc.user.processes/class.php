...
use Diag\Logger\FileHandlerLog;
use Home\Workflow\CustomWorkflowStateToGet;
	...
	implements \Bitrix\Main\Errorable, \Bitrix\Main\Engine\Contract\Controllerable
{
	...
	/**
	 * Возвращает массив сортировки для запроса к WorkflowStateService
	 * @return array
	 */
	private function getGridOrder(): array
	{
		...
		$sorting = $gridOptions->getSorting([
			'sort' => ['ID' => 'DESC'], // сортировка по умолчанию (можно изменить или убрать)
			'vars' => ['by' => 'by', 'order' => 'order']
		]);

		$order = $sorting['sort'];

		// Маппинг полей грида на поля сущности WorkflowState
		$fieldMap = [
			// с левой стороны поле в таблице БП => с права поле в БД
			'ID'               		=> 'WORKFLOW_ID',
			'WORKFLOW_STARTED'   	=> 'MODIFIED',
		];

		$result = [];
		foreach ($order as $field => $direction) {
			if (isset($fieldMap[$field])) {
				$result[$fieldMap[$field]] = strtoupper($direction) === 'DESC' ? 'DESC' : 'ASC';
			}
		}
	...
	private function fetchWorkflows(int $limit, int $offset, bool $shouldCountTotal = true): GetListResponse
	{
		...
		$ormFilter = $workflowsRequest->getOrmFilter();
	...
	// private function fetchWorkflows(int $limit, int $offset, bool $shouldCountTotal = true): \Bitrix\Bizproc\Api\Response\WorkflowStateService\GetListResponse
	// {
	// 	$workflowsRequest = (new \Bitrix\Bizproc\Api\Data\WorkflowStateService\WorkflowStateToGet())
	// 		->setAdditionalSelectFields(static::WORKFLOW_FIELDS_TO_LOAD)
	// 		->setLimit($limit)
	// 		->setOffset($offset)
	// 	;
	...
	// 	// 1. ОЧИСТКА ФИЛЬТРА ПОД WorkflowStateTable
	// 	if (isset($ormFilter['=USER_ID'])) 
	// 	{
	// 		// Если USER_ID > 0, фильтруем по автору процесса
	// 		if ($ormFilter['=USER_ID'] > 0)
	// 		{
	// 			$ormFilter['=STARTED_BY'] = $ormFilter['=USER_ID'];
	// 		}
	// 		unset($ormFilter['=USER_ID']);
	// 	}

	// 	// Добавляем обязательные поля для формирования DocumentId
	// 	$systemFields = [
	// 		'ID', 
	// 		'MODIFIED', 
	// 		'STARTED', 
	...
	// 	$query = \Bitrix\Bizproc\Workflow\Entity\WorkflowStateTable::getList([
	// 		// Объединяем системные поля и поля из константы компонента
	// 		'select' => array_unique(array_merge($systemFields, static::WORKFLOW_FIELDS_TO_LOAD)),
	// 		// 'filter' => $ormFilter,
	...
	// 	// 3. СБОРКА КОЛЛЕКЦИИ ДЛЯ ОТВЕТА
	// 	$response = new \Bitrix\Bizproc\Api\Response\WorkflowStateService\GetListResponse();
	// 	$responseCollection = new \Bitrix\Bizproc\Workflow\Entity\EO_WorkflowState_Collection();

	// 	foreach ($query->fetchCollection() as $workflowState) 
	// 	{
	// 		$responseCollection->add($workflowState);
	// 		$response->setUserModified($workflowState->getId(), $workflowState->getModified());
	// 	}

	// 	if ($shouldCountTotal) 
	// 	{
	// 		$response->setTotalCount(\Bitrix\Bizproc\Workflow\Entity\WorkflowStateTable::getCount($ormFilter));
	// 	}
